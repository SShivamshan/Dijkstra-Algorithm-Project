...
std::vector<std::pair<uint64_t,uint64_t> > Station_parser::reverse_path(uint64_t _start, uint64_t _end,std::unordered_map<uint64_t,uint64_t> &prev){
    ...
    std::pair<uint64_t,uint64_t> pos(_start,temps);
...
vector<std::pair<uint64_t,uint64_t>> Station_parser::compute_travel(uint64_t _start, uint64_t _end){
    ...
    while(!unvisited.empty()){
        ...
        // if the current node is equal to the end node 
        if(current == _end){
            paths = this->reverse_path(_start,_end,prev);
            return paths;
        }
